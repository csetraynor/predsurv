...
survdata <- surv_sim_data(N = 20, features = 50, mu = 3, alpha = 0.8)
```
...
```{r}
survdata <- surv_sim_data(N = 20, features = 50, mu = -3, alpha = 0.8, cens_rate = 1/10)
...
mod.fs.coxph = fun_fit(train, fit = "stepwise")
pred.fs.coxph = mod_pred(obj = mod.fs.coxph, beta = coef)
brier_score = fun_brier_score(obj = mod.fs.coxph, pred = pred.fs.coxph, data = test , beta = coef)
...
```{r}
cv.ibrier.fs = fun_cv(data = survdata, method = "forward", K = 20) 
print(cv.ibrier.fs)
```

Now we can plot the predictions of the model using the function survfit, we also can make use of the "then" operator from magrittr "%>%":
```{r}
pred.fs.coxph = mod_pred(obj = mod.fs.coxph, beta = coef)
```
...
x <- as.matrix(train %>% dplyr::select(dplyr::contains("feature")))
y <- as.matrix(train %>% 
                 dplyr::mutate(status = os_status == "DECEASED" ,
                               time = os_months) %>%
...
```{r}
lasso_cv_fit <- glmnet::cv.glmnet(x, y , family = "cox", fold = 10)
```
...
Lets consider now the random forest we can fit the model with.
```{r}
mod.bst = fun_fit(train, fit = "random forest")
random_forest_plot(mod.bst)
```

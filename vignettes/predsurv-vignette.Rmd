...
```{r}
survdata <- surv_sim_data(N = 100, features = 100)
plot_km(survdata, time = os_months, status = os_deceased, event_type = 1)
```
...
mod.uni = fun_fit(train, fit = "Univariate")
brier_score = pred_error(obj = mod.uni, fit = "Univariate", pred = "Brier")
plot_brier(brier_score)
```

Calculate the c-index:
```{R}
c_index = pred_error(obj = mod.uni, fit = "Univariate", pred = "c_index")
print(c_index)
```

```{r}
uni.roc = pred_error(obj = mod.uni, fit = "Univariate", pred = "ROC")
...
brier_score = pred_error(obj = mod.fs, fit = "stepwise")
plot_brier(brier_score)
```
...
brier_score = pred_error(obj = mod.pcr, fit = "PCR")
plot_brier(brier_score)
```

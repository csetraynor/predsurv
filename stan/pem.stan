...
P          = number of covariate parameters

// main data matrix (per observed timepoint*record)
s          = sample id for each obs
t          = timepoint id for each obs
event      = integer indicating if there was an event at time t for sample s
x          = matrix of real-valued covariates at time t for sample n [N, P]
...
*/
  ...
  data {
    ...
    matrix[N, P] x //n x p  data matrix
...
parameters {
  ...
  vector[P] beta;         // beta for each covariate

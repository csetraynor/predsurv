...
survdata <- surv_sim_data(n = 200, features = 1000)
```
...
```{r}
fold = create_training_test_set(survdata, p = 0.8, status = os_status)
train = fold[["train"]]
test = fold[["test"]]
head(train[,1:6])
```
...
fit_forward = MASS::stepAIC(survival::coxph(Surv(os_months, os_deceased) ~ ., data = train ), direction = "forward")
print(fit_forward)
```
...
```{r}
pred_coxph = pred_surv(obj = fit_forward, beta = coef)
plot_km(pred_coxph, time = time, status = status, event_type = 1)
plot_km(test, time = os_months, status = os_deceased, event_type = 1)
```

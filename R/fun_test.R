  ...
  if(fit != "Random forest"){
    ...
    if(pred == "Brier" | all){
      if(length(selectedBeta) == 0){
        ...
        if(integrated){
          if(reference){
            out <- predsurv::fun_ibrier_score_reference(out)
          }else{
            out <- predsurv::fun_ibrier_score_model(out)
          }
        }
      }
    }
    if(pred == "ROC" | all ){
      if(length(selectedBeta) == 0){
        ...
        if(fit == "Ridge regression"){
          probs <- as.matrix(testX, ncol = length(selectedBeta)) %*% inits
        }else{
      ...
      roc <- tdROC::tdROC(X = probs,
                          Y = test_data %>%
                            ...
                            unlist,
                          delta = test_data %>%
                            dplyr::select(status = !!status)%>%
                            unlist,
                          tau = quantile(test_data %>%
                                           dplyr::select(time = !!time) %>%
                                           unlist, .73),
                          nboot = noboot,
                          alpha = 0.05,
...
#' @importFrom rlang !!
#' @import prodlim
fun_ibrier_score_model <- function(brier){

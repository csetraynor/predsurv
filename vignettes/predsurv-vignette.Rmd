...
```{r}
survdata <- get_data()
plot_km(hei, time = time, status = cens, event_type = 1)
```

```{r}
fold = create_training_test_set(survdata, p = 0.8, status = status)
train = fold[["train"]]
test = fold[["test"]]
...
mod.uni = fun_fit(train, fit = "Univariate", time = os, status = status)
uni.brier_score = pred_error(obj = mod.uni, fit = "Univariate", pred = "Brier", time = os, status = status)
plot_brier(uni.brier_score)
```

Calculate the c-index:
```{R}
c_index = pred_error(obj = mod.uni, fit = "Univariate", pred = "c_index", status = status, time = os)
print(uni.c_index)
```

```{r}
uni.roc = pred_error(obj = mod.uni, fit = "Univariate", pred = "ROC",  status = status, time = os)
tdROC::plot.tdROC(uni.roc)
```

The forward step wise method can be fitted using the function stepAIC in the MASS package:
```{r, results="hide", eval = FALSE, error = FALSE, warning=FALSE}
mod.fs = fun_fit(train, fit = "stepwise")
fs.brier_score = pred_error(obj = mod.fs, fit = "stepwise")
plot_brier(brier_score)
```

Now we can obtain the cumulative prediction error curve or integrated Brier Score :
```{r}
ibrier = fun_ibrier_score(brier_score)
print(ibrier)
```
...
```{r}
mod.pls = fun_fit(train, fit = "PLS")
brier_score = pred_error(obj = mod.pls, fit = "PLS")
```
...
```{r}
mod.lasso = fun_fit(train, fit = "lasso")
plot(mod.lasso)
```
...
```{r}
brier_score = pred_error(obj = mod.lasso, fit = "lasso", pred = "Brier")
...
```{r}
bst = fun_fit(train, fit = "random forest")
```

Let's calculate the Brier score:
```{R}
brier_score = pred_error(obj = bst, fit = "random forest", pred = "Brier")
plot_brier(brier_score)
```
